...
const CrudwithHooks = () => {
    ...
    const handleChange1=(e)=>{
        let fnamereg=/^[a-z ,.'-]+$/i
        let res1=fnamereg.test(e.target.value)
        if(!res1){
                setErr("Wrong Input")
        }else {
            setErr("Correct")
...
const handleEdit=(index)=>{
    ...
    return (
        <div className="container">
            ...
            <button className='btn btn-dark ml-3' onClick={handleClear}>CLEAR</button>
            <button className="btn btn-secondary ml-3" onClick={handleDelAll}>Delete All</button><hr/> 
